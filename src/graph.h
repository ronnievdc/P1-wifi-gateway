...
void sendStatistics(const String &type, const String &period) {
  ...
  if (period == "day") { // past 24 to 48 hours
    ...
    statistics.sendDays(statStream, startOfWeekBefore, startOfToday + SECS_PER_DAY);
...
void GraphJS(const char index, const char *type, String title, String label, String period) {
  ...
  String str = "";
  str = F("google.charts.setOnLoadCallback(drawChart") + idx + F(");");
  str += F("function drawChart") + idx + F("() {");
  str += F("var rawData = [");
  server.sendContent(str);

  delay(200);
  sendStatistics(String(type), period);
  delay(200);
  ...
  if (type[0] == 'T') {
    ...
    str += F("if (currentIndex !== 0) { const date = new Date(prevDate * 1000);");
    if (period != "day") {
      str += F("date.setHours(0);");
    }
    str += F("date.setMinutes(0, 0, 0); accumulator.push([date, currentValue[1] - prevValue]);}");
  ...
  // Format the tooltips
  str = F("var formatter = new google.visualization.DateFormat({pattern: '") + xAxisFormat + F("'});");
  str += F("formatter.format(data, 0);");

  str += F("var options = {title: '") + title + F("', hAxis: {format: '") + xAxisFormat + F("', slantedText: true, slantedTextAngle: 90, gridlines: {count: rows.length}}};");
  if (type[0] == 'T')
    str +=
        F("var chart = new "
          "google.visualization.LineChart(document.getElementById('Chart") + idx + F("'));");
  else
    str += F(
        "var chart = new "
        "google.visualization.ColumnChart(document.getElementById('Chart") + idx + F("'));");
...
void theGraph(const char *type1, const char *type2, String title1,
  ...
  str += F("<script type=\"text/javascript\">google.charts.load('current', "
           "{'packages':['corechart']});");
  server.sendContent(str);
  graphJS('1', type1, title1, label, period);
  if (type2[0] != '\0') {
    delay(200);
    graphJS('2', type2, title2, label, period);
  }
  delay(200);
